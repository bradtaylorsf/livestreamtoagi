...
- async PostgreSQL and Redis clients with a typed repository layer (9 repo classes including artifact, simulation, and eval repos)
...
- raw SQL migrations (13 files, up to `013_eval_tables`) and typed repository classes
...
- a simulation orchestrator (`core/simulation/`) with clock, phases, display, and CLI entry point (`scripts/run_simulation.py`)
- an eval engine (`core/eval/`) with eval loader, prompt loader, and engine for running post-simulation evaluations
...
- Data layer: PostgreSQL 16 with `pgvector` and `pg_trgm`, plus 13 raw SQL migrations under `db/migrations`
...
core/repos/             Typed repository layer: agents, conversations, memory, costs, transcripts, world, artifacts, simulations, evals
core/simulation/        Simulation orchestrator: clock, phases, display, orchestrator
core/eval/              Evaluation engine: loader, prompt_loader, engine
db/                     Raw SQL migration runner, init SQL, and numbered up/down migrations (001–013)
...
scripts/                Utility scripts: chat.py, test_agent.py, watch_conversations.py, run_simulation.py, run_eval.py, check_tool_coverage.py, check-services.sh
...
tests/backend/          ~52 test files covering admin routes, agent registry, conversation engine, memory tiers, event bus, context assembly, repos, migrations, LLM client, reflection, tools, TTS, speech parsing, simulations, artifacts, evals
```
...
- `core/admin_routes.py` provides 32 admin endpoints: agent inspection, conversation viewer, artifact browser, simulation timeline, eval dashboard, transcript viewer, system prompt inspection
...
- `db/migrate.py` is the raw SQL migration entry point for `python -m db`; migrations go up to `013_eval_tables`
