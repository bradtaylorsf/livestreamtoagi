...
- a FastAPI backend with a health route, WebSocket event bus, and lifespan hooks that initialize database, Redis, agent registry, memory managers, LLM client, TTS pipeline, and a reflection scheduler
...
- raw SQL migrations (6 files, up to `006_self_modification_fields`) and typed repository classes
...
- Data layer: PostgreSQL 16 with `pgvector` and `pg_trgm`, plus 6 raw SQL migrations under `db/migrations`
...
db/                     Raw SQL migration runner, init SQL, and numbered up/down migrations (001–006)
...
scripts/                Utility scripts: chat.py (interactive agent chat), test_agent.py (agent harness + reflection runner), watch_conversations.py (real-time monitor), check-services.sh
...
tests/backend/          ~42 test files covering agent registry, conversation engine, memory tiers, event bus, context assembly, repos, migrations, LLM client, reflection, tools, TTS, speech parsing
```
...
- `db/migrate.py` is the raw SQL migration entry point for `python -m db`; migrations go up to `006_self_modification_fields`
...
- The conversation engine, memory system, reflection scheduler, tools, and LLM client are initialized at startup, but there is no production entry point driving continuous agent dialogue yet; CrewAI is installed but not wired
