...
- a FastAPI backend with a health route, WebSocket event bus, admin API (32 endpoints), and lifespan hooks that initialize database, Redis, agent registry, memory managers, LLM client, TTS pipeline, config watcher, and a reflection scheduler
- async PostgreSQL and Redis clients with a typed repository layer (8 repo classes including artifact and simulation repos)
...
- a conversation engine orchestrator with speaker selection, energy model, interrupts, topic detection, pacing, proximity groups, Overseer safety review, and TTS output
- a context assembly pipeline that builds three-layer prompts: infrastructure rules → character identity → mutable memory state
- a tool registry with 18 implemented tools: messaging, audience interaction, memory operations, code execution (Docker sandbox), tilemap generation, revenue/social drafts, web search, Alpha dispatch, and self-modification
- raw SQL migrations (12 files, up to `012_journal_conversation_type`) and typed repository classes
...
- an admin dashboard backend (`core/admin_routes.py`) with agent inspection, conversation viewer, artifact browser, simulation timeline, and transcript viewer — protected by `ADMIN_PASSWORD` Bearer auth
- a simulation orchestrator (`scripts/run_simulation.py`) for running full-day simulations via CLI
- a small Vite TypeScript frontend package with agent constants/tests
- a Next.js website with static route pages, admin dashboard UI (agent inspector, conversation viewer, artifact browser, simulation timeline), shared TS types, and an API client that targets future backend routes not yet implemented
...
- Conversation: `core/conversation_engine.py` orchestrates speaker selection, energy, interrupts, Overseer review, TTS, and event emission; subsystems in `core/conversation/` (energy, pacing, proximity, speaker_selector, topic_detector, triggers, selection_logger)
...
- Overseer: `core/overseer.py` reviews all agent output before broadcast (content filter with intervention levels)
- Scheduling: APScheduler runs reflection cycles (6-hour at 2/8/14/20 UTC, weekly Sunday 20 UTC) via `core/scheduler.py`
- Tools: `tools/` package with 18 tool classes and a `ToolRegistry`; includes code execution via Docker sandbox with gVisor
- Data layer: PostgreSQL 16 with `pgvector` and `pg_trgm`, plus 12 raw SQL migrations under `db/migrations`
- Realtime: WebSocket event bus in `core/event_bus.py` (16 event types, 50-message history buffer, max 100 connections)
...
core/repos/             Typed repository layer: agents, conversations, memory, costs, transcripts, world, artifacts, simulations
db/                     Raw SQL migration runner, init SQL, and numbered up/down migrations (001–012)
frontend/               Small TypeScript package; currently agent constants/tests, not a full Phaser world app
website/                Next.js app router site with public pages and admin dashboard (agent inspector, conversation viewer, artifact browser, simulation timeline)
scripts/                Utility scripts: chat.py, test_agent.py, watch_conversations.py, run_simulation.py, check-services.sh
specs/                  Product and architecture reference docs; useful context, not the runtime source of truth
tools/                  Agent tool implementations: 18 tool classes + ToolRegistry (messaging, audience, memory, code execution, tilemap, revenue, web, Alpha dispatch, self-modification)
tests/backend/          ~46 test files covering admin routes, agent registry, conversation engine, memory tiers, event bus, context assembly, repos, migrations, LLM client, reflection, tools, TTS, speech parsing, simulations, artifacts
```
...
- `core/admin_routes.py` provides 32 admin endpoints: agent inspection, conversation viewer, artifact browser, simulation timeline, transcript viewer, system prompt inspection
...
- `core/conversation_engine.py` is the central runtime loop — ties together triggers, speaker selection, energy, interrupts, Overseer review, TTS, and event emission
...
- `tools/__init__.py` exports all 18 tools and the `ToolRegistry` with `get_core_tools()` factory
- `db/migrate.py` is the raw SQL migration entry point for `python -m db`; migrations go up to `012_journal_conversation_type`
...
- The admin dashboard backend (`/api/admin/*`) is live with 32 endpoints; the public-facing API routes called from `website/src/lib/api.ts` are not yet served by the backend
- The conversation engine, memory system, reflection scheduler, tools, and LLM client are initialized at startup, but there is no production entry point driving continuous agent dialogue yet; use `scripts/run_simulation.py` for full simulations; CrewAI is installed but not wired
