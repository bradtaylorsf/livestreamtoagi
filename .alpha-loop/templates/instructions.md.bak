...
Now I have all the accurate counts. Let me produce the updated file.
...
- a FastAPI backend with a health route, WebSocket event bus, admin API (55 endpoints), and lifespan hooks that initialize database, Redis, agent registry, memory managers, LLM client, TTS pipeline, config watcher, and a reflection scheduler
...
- an admin dashboard backend (`core/admin_routes.py`) with 55 endpoints: agent inspection, conversation viewer, artifact browser, simulation timeline, eval dashboard, transcript viewer, config management, evolution tracking — protected by `ADMIN_PASSWORD` Bearer auth
...
- Data layer: PostgreSQL 16 with `pgvector` and `pg_trgm`, plus 30 raw SQL migration pairs under `db/migrations`
- Realtime: WebSocket event bus in `core/event_bus.py` (23 event types, 50-message history buffer, max 100 connections)
...
db/                     Raw SQL migration runner, init SQL, and numbered up/down migrations (001–030)
...
scripts/                Utility scripts: chat.py, test_agent.py, watch_conversations.py, run_simulation.py, run_eval.py, run_evolution.py, run_reflection_test.py, report_simulation.py, snapshot_memory.py, restore_memory.py, seed_config.py, check_tool_coverage.py, check-services.sh, generate_office_tilemap.py
...
tools/                  Agent tool implementations: 15 tool modules + ToolRegistry (messaging, audience, memory, code execution, tilemap, revenue, web, Alpha dispatch, self-modification, task management, world state, evolution log, alliances, economy, character proposals)
tests/                  ~89 test files: backend/ (unit + integration), frontend/ (vitest), website/ (vitest + playwright e2e)
```
...
- `core/admin_routes.py` provides 55 admin endpoints: agent inspection, conversation viewer, artifact browser, simulation timeline, eval dashboard, transcript viewer, config management, evolution tracking
...
- `core/models.py` is the source of backend Pydantic schemas (~101 model classes: agents, memory tiers, conversations, transcripts, journal entries, self-modification proposals, LLM responses, cost events, goals, relationships, prompt logs)
...
- `db/migrate.py` is the raw SQL migration entry point for `python -m db`; migrations go up to `030_goal_source_dream`
...
- The admin dashboard backend (`/api/admin/*`) is live with 55 endpoints; the public-facing API routes called from `website/src/lib/api.ts` are not yet served by the backend
