...
async def test_redis_client_retries_with_default_dev_password_when_auth_required(
    monkeypatch,
) -> None:
    calls: list[str] = []
    ...
    assert calls == [
        ...
        "redis://:devpassword@localhost:6381",
    ]
...
async def test_redis_client_uses_env_password_for_auth_retry(monkeypatch) -> None:
    ...
    assert calls == [
        "redis://localhost:6381",
        "redis://:custom%20password@localhost:6381",
    ]
