...
def test_phase_runner_summaries_buffer():
    ...
    assert runner._conversation_summaries.maxlen == 10
    assert isinstance(runner._recent_outputs, deque)
    assert runner._recent_outputs.maxlen == 50

    # Simulate adding summaries — deque(maxlen=10) handles trimming automatically
    for i in range(30):
        runner._conversation_summaries.append(f"Summary {i}")

    assert len(runner._conversation_summaries) == 10
    assert runner._conversation_summaries[0] == "Summary 20"
