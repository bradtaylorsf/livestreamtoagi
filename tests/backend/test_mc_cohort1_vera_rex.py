"""Tests for E8-2 Vera/Rex embodied Mindcraft cohort.
...
COHORT = {
    "vera": {
        "name": "Vera",
        "script": REPO_ROOT / "scripts" / "minecraft" / "connect-vera-bot.sh",
        "settings": REPO_ROOT / "scripts" / "minecraft" / "mindcraft-settings-vera.js",
        "profile": REPO_ROOT / "scripts" / "minecraft" / "profiles" / "vera-bot.json",
    },
    "rex": {
        "name": "Rex",
        "script": REPO_ROOT / "scripts" / "minecraft" / "connect-rex-bot.sh",
        "settings": REPO_ROOT / "scripts" / "minecraft" / "mindcraft-settings-rex.js",
        "profile": REPO_ROOT / "scripts" / "minecraft" / "profiles" / "rex-bot.json",
    },
}
...
def _load_gen_module():
    spec = importlib.util.spec_from_file_location("mc_gen_profiles_cohort1", GEN_SCRIPT)
...
def _claude_model_label(raw_model_id: str) -> str:
    ...
    return raw_model_id.split("/", 1)[1].replace("-", " ").title()
