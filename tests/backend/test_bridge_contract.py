...
ALLOWED_REGISTRY_KEYS = ADR_SERVICE_NAMES | {"bridge.ping", "errand.poll"}
...
def test_protocol_version_is_self_consistent() -> None:
    # 1.5: E7-2 (#566) added errand.poll — an
    # additive minor bump (ADR §3), same major as earlier 1.x peers.
    assert c.PROTOCOL_VERSION == "1.5"
    assert c.is_supported_version(c.PROTOCOL_VERSION)
    assert c.parse_version(c.PROTOCOL_VERSION) == (1, 5, 0)


@pytest.mark.parametrize("version", ["1.0", "1.3", "1.4", "1.5", "1.0.9", "1.99.99"])
...
def test_registry_is_closed_and_uses_adr_names_only() -> None:
    ...
    assert not unexpected, (
        ...
        f"(+bridge.ping/+errand.poll): {sorted(unexpected)}"
    )
